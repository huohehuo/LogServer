 ...
 * 库存查询VUE版的接口
 */
@WebServlet(urlPatterns = "/GetCompany4Web")
public class GetCompany4Web extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Connection conn = null;
        String json = "";
        PreparedStatement sta = null;
        ResultSet rs = null;
        Lg.e("进入查询GetCompany4Web");
        response.setCharacterEncoding("UTF-8");
        String seach =request.getParameter("seach");

        CompanyDao companyDao = new CompanyDao();
        StatisticalDao statisticalDao = new StatisticalDao();
        ArrayList<Company> stsList = companyDao.findCompanyByName(seach);
        for (int i = 0; i < stsList.size(); i++) {//获取每个项目的总人数
            stsList.get(i).has_user_num = statisticalDao.getStatisticalNum4Appid(stsList.get(i).AppID);
        }
        CommonResponse4Web commonResponse4Web = new CommonResponse4Web(true,stsList.size());
        commonResponse4Web.companies = stsList;
